...
class DataVisualization(object):
    ...
    def read_csv(self, file_path):
        self.df = pd.read_csv(file_path, header = 2, encoding = 'UTF-8', skiprows=[3])

    def drop_df(self, start_index, end_index):
        drop_index_list = list(range(start_index, end_index))
        self.df.drop(index=self.df.index[drop_index_list], inplace=True)
    ...
    def set_text(self):
        ...
        ax.text(0, 0.4, 'datetime: ' + str(self.df['TIME (UTC+09:00)'][0]), fontsize=14)
...
def main():
    ...
    ins.insert(0, 'eTime', [n / 1000000 for n in range(0, 4000*ins.get_size(), 4000)])
    ...
    ins.set_text()
    ins.set_graph_config(config2.plc_graph_config)
